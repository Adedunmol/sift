...
// for the given partIndex, matching the format of a real Twitter archive file.
func archiveReader(partIndex int, jsonArray string) io.ReadSeeker {
	src := fmt.Sprintf("window.YTD.tweets.part%d = ", partIndex) + jsonArray
...
func singleTweetArchive(id, text string) io.ReadSeeker {
	return archiveReader(0, "["+rawTweetJSON(id, text)+"]")
}
...
func multiTweetArchive(tweets [][2]string) io.ReadSeeker {
	...
	return archiveReader(0, "["+strings.Join(parts, ",")+"]")
}
...
func TestNewStream_ValidPrefixSucceeds(t *testing.T) {
	...
	_, err := parser.NewStream(r, 0, 0, "user")
	if err != nil {
...
func TestNewStream_Part1PrefixSucceeds(t *testing.T) {
	r := archiveReader(1, `[`+rawTweetJSON("1", "hello")+`]`)
	_, err := parser.NewStream(r, 1, 0, "user")
	if err != nil {
		t.Fatalf("unexpected error for part1 prefix: %v", err)
	}
}

func TestNewStream_WrongPartIndexReturnsError(t *testing.T) {
	// Archive has part0 prefix but NewStream is told partIndex=1.
	r := singleTweetArchive("1", "hello")
	_, err := parser.NewStream(r, 1, 0, "user")
	if err == nil {
		t.Fatal("expected error when partIndex does not match archive prefix, got nil")
	}
}
...
func TestNewStream_StartOffsetSeeksCorrectly(t *testing.T) {
	...
	s1, err := parser.NewStream(r, 0, 0, "user")
	if err != nil {
		t.Fatalf("s1: %v", err)
	}
	...
	s2, err := parser.NewStream(r, 0, cp, "user")
	if err != nil {
		t.Fatalf("s2: %v", err)
	}
...
func TestNewStream_InvalidSeekOffsetReturnsError(t *testing.T) {
	...
	_, err := parser.NewStream(r, 0, 99, "user")
	if err == nil {
...
func (e *errSeeker) Seek(int64, int) (int64, error) { return 0, io.ErrUnexpectedEOF }
...
func TestNext_PopulatesUsername(t *testing.T) {
	...
	s, _ := parser.NewStream(r, 0, 0, "alice")
...
func TestNext_DecodesMultipleTweetsInOrder(t *testing.T) {
	r := multiTweetArchive([][2]string{{"10", "first"}, {"20", "second"}, {"30", "third"}})
	s, _ := parser.NewStream(r, 0, 0, "")
	...
	for i, wantID := range []int64{10, 20, 30} {
		if tweets[i].ID != wantID {
			t.Errorf("tweet[%d].ID = %d, want %d", i, tweets[i].ID, wantID)
		}
	}
}

func TestNext_DecodesTweetWithSpecialCharactersInText(t *testing.T) {
	r := archiveReader(0, `[{"tweet":{"id":"1","full_text":"hello\nworld\t\u0026 \"quoted\""}}]`)
	s, _ := parser.NewStream(r, 0, 0, "")
	tw, err := s.Next(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestNext_NonNumericIDReturnsError(t *testing.T) {
	r := archiveReader(0, `[{"tweet":{"id":"not-a-number","full_text":"text"}}]`)
	s, _ := parser.NewStream(r, 0, 0, "")
	_, err := s.Next(context.Background())
	if err == nil {
...
func TestNext_EmptyIDReturnsError(t *testing.T) {
	r := archiveReader(0, `[{"tweet":{"id":"","full_text":"text"}}]`)
	s, _ := parser.NewStream(r, 0, 0, "")
	_, err := s.Next(context.Background())
	if err == nil {
...
func TestNext_MalformedJSONReturnsError(t *testing.T) {
	r := archiveReader(0, `[{"tweet":{"id":1,"full_text":"text"}}`) // unclosed array
	s, _ := parser.NewStream(r, 0, 0, "")
	s.Next(context.Background())
	_, err := s.Next(context.Background())
	if err == nil {
...
func TestOffset_AdvancesAfterEachNext(t *testing.T) {
	r := multiTweetArchive([][2]string{{"1", "first"}, {"2", "second"}, {"3", "third"}})
	s, _ := parser.NewStream(r, 0, 0, "")
...
func TestOffset_MatchesCheckpointAfterResume(t *testing.T) {
	r := multiTweetArchive([][2]string{{"100", "first"}, {"200", "second"}})

	s1, _ := parser.NewStream(r, 0, 0, "")
	s1.Next(context.Background())
	cp := s1.Offset()

	s2, err := parser.NewStream(r, 0, cp, "")
	if err != nil {
...
func TestOffset_DoesNotChangeOnEOF(t *testing.T) {
	...
	s.Next(context.Background())
	afterLast := s.Offset()
	s.Next(context.Background()) // EOF
	if s.Offset() != afterLast {
		t.Errorf("Offset changed after EOF: before=%d after=%d", afterLast, s.Offset())
	}
}
...
func TestURL_WithUsername(t *testing.T) {
	...
	if got := tw.URL(); got != "https://twitter.com/alice/status/123" {
		t.Errorf("URL() = %q", got)
	}
}

func TestURL_WithoutUsername(t *testing.T) {
	tw := parser.Tweet{ID: 456}
	if got := tw.URL(); got != "https://twitter.com/i/web/status/456" {
		t.Errorf("URL() = %q", got)
	}
}
...
func TestURL_ZeroIDWithUsername(t *testing.T) {
	...
	if got := tw.URL(); got != "https://twitter.com/carol/status/0" {
		t.Errorf("URL() = %q", got)
	}
}
...
func TestNext_DecodesLargeArchiveWithCorrectCount(t *testing.T) {
	const n = 10_000
	s, err := parser.NewStream(generateArchive(t, n), 0, 0, "user")
	if err != nil {
		t.Fatalf("NewStream: %v", err)
	}
	if got := len(drainStream(t, s)); got != n {
		t.Errorf("decoded %d tweets, want %d", got, n)
	}
}

func TestNext_LargeArchiveAllIDsUnique(t *testing.T) {
	const n = 10_000
	s, _ := parser.NewStream(generateArchive(t, n), 0, 0, "")
...
func TestNext_LargeArchiveUsernameConsistentOnEveryTweet(t *testing.T) {
	const n = 5_000
	s, _ := parser.NewStream(generateArchive(t, n), 0, 0, "archiveuser")
	for i := 0; ; i++ {
		tw, err := s.Next(context.Background())
		if err == io.EOF {
			break
...
func TestSeek_ResumeMidArchiveYieldsCorrectRemainingTweets(t *testing.T) {
	...
	for i := range tweets {
		tweets[i] = [2]string{fmt.Sprintf("%d", i+1), fmt.Sprintf("text %d", i+1)}
	}
	...
	for i, tw := range remaining {
		if wantID := int64(i + 5); tw.ID != wantID {
...
func TestSeek_ResumeFromFirstCheckpointSkipsOnlyFirstTweet(t *testing.T) {
	...
	if remaining[0].ID != 2 || remaining[1].ID != 3 {
		t.Errorf("IDs = [%d %d], want [2 3]", remaining[0].ID, remaining[1].ID)
	}
}

func TestSeek_ResumeFromLastCheckpointYieldsEOFImmediately(t *testing.T) {
	r := multiTweetArchive([][2]string{{"1", "only"}, {"2", "last"}})
...
func TestSeek_ResumeAtEachCheckpointProducesCorrectTail(t *testing.T) {
	...
	for resumeAfter, cp := range checkpoints {
		s, err := parser.NewStream(r, 0, cp, "")
		if err != nil {
		...
		if len(got) != wantCount {
			t.Errorf("resume after tweet %d: got %d tweets, want %d", resumeAfter+1, len(got), wantCount)
			continue
		}
		for i, tw := range got {
			if wantID := int64(resumeAfter + i + 2); tw.ID != wantID {
				t.Errorf("resume after tweet %d: got[%d].ID = %d, want %d", resumeAfter+1, i, tw.ID, wantID)
			}
		}
	}
}
...
func TestSeek_LargeArchiveResumeFromMidpoint(t *testing.T) {
	const total, resumeAfter = 2_000, 1_000
	tweets := make([][2]string, total)
	for i := range tweets {
	...
	if len(second) != total-resumeAfter {
		t.Fatalf("expected %d tweets, got %d", total-resumeAfter, len(second))
	}
	if second[0].ID != resumeAfter+1 {
		t.Errorf("first resumed ID = %d, want %d", second[0].ID, resumeAfter+1)
	}
	if second[len(second)-1].ID != total {
		t.Errorf("last resumed ID = %d, want %d", second[len(second)-1].ID, total)
	}
}
